...
`
  ...
  async updateRepos (repo) {
    ...
    let isArchived, shouldArchive, shouldUnarchive
    try {
      ({ isArchived, shouldArchive, shouldUnarchive } = await archivePlugin.getState())
      ...
      if (shouldUnarchive) {
        ...
        // Archived repos are read-only on GitHub - any other plugin call would just 403/422.
        this.log.debug(`Repo ${repo.repo} is archived, skipping settings sync`)
        return
      }
    } catch (e) {
      if (this.nop) {
